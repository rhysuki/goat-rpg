...
function lust.print(...)
  if lust.is_printing_enabled then print(...) end
end

function lust.describe(name, fn)
  table.insert(lust.level_names, name)

  lust.print(indent() .. name)
...
function lust.it(name, fn)
  ...
  if success then lust.passes = lust.passes + 1
    ...
    local error_path = table.concat(lust.level_names, ': ')
    local indented_error = err and ('\n\t' .. err) or ''
    table.insert(lust.error_messages, error_path .. ' @ "' .. name .. '"' .. indented_error)
  end
  local color = success and green or red
  local label = success and 'PASS' or 'FAIL'
  lust.print(indent() .. color .. label .. normal .. ' ' .. name)
  if err then
    lust.print(indent(lust.level + 1) .. red .. err .. normal)
  end
...
local paths = {
  ...
  fail = {
    test = function(v)
      ...
      local error_info = (not success) and err or 'no error message given'

      return not success,
        'expected ' .. tostring(v) .. ' to fail',
        'expected ' .. tostring(v) .. ' to not fail (' .. error_info .. ')'
    end
  }
}
